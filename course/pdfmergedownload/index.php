...
//require_once 'lang/en/course_pdfmergedownload.php';
...
$strdownloadall  = get_string('downloadall');
...
//$strpluginname   = get_string('pluginname','course_pdfmergedownload');

$PAGE->set_url('/course/pdfmergedownload/index.php', array('id' => $course->id));
$PAGE->set_title($course->shortname.': '.$strdownload);
$PAGE->set_heading($course->fullname);
$PAGE->navbar->add($strdownload);

echo $OUTPUT->header();
echo $OUTPUT->heading($strdownloadall);
//echo $strpluginname;
...
if ($usesections) {
	$strsectionname = get_string('sectionname', 'format_'.$course->format);
	$table->head  = array ($strsectionname, $strname, $strintro, $strsize.' ('.$strsizeb.')', $strlocation);
...
foreach ($cms as $cm) {
	...
	if (count($files) < 1) {
		...
		$file = reset($files);	// reset(array &$array) - Sets the internal pointer of an array to its first element
		unset($files);
	}
	...
	$file_chash = $file->get_contenthash();
	
	static $p = 0;
	$loc[$p] = substr($file_chash,0,2).'/'.substr($file_chash,2,2).'/'.$file_chash;
...
// merge all course pdf files and store the merged document at a temporary location
	...
	// merge pdf files in the course using pdftk
	$cmd = "pdftk ";
	...
	echo "<br> CHash : ".$mergedfile->get_contenthash()."<br>";
...
// create a blank numbered pdf document --------------------------------------------------------------------
	...
	*/
	...
	$latexfileinfo = array(
			...
			'itemid' 	=> 0,               	// usually = ID of row in table
			'filepath' 	=> '/',           		// any path beginning and ending in /
			'filename' 	=> $tempfilename.'.tex'); 	// any filename
...
// merge the blank numbered pdf document with the merged pdf document (containing all course pdfs)
	...
	$stampedfileinfo = array(
			...
			'itemid' 	=> 0,               	// usually = ID of row in table
			'filepath' 	=> '/',           		// any path beginning and ending in /
			'filename' 	=> uniqid('stampedfile_').'.pdf'); 	// any filename
	...
	echo "<br> Stamped File | "."<a $class $extra href=\"".$stampedfileurl."\">".$icon.$stampedfile->get_filename()."</a>";
	echo "<br> CHash : ".$stampedfile->get_contenthash()."<br>";
